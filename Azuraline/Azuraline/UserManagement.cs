...
namespace Azuraline
{
    public partial class UserManagement : Form
    {
        ...
        private void DataTableUser_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0 && e.RowIndex < DataTableUser.Rows.Count)
            {
                DataGridViewRow selectedRow = DataTableUser.Rows[e.RowIndex];
                if (selectedRow.Cells[e.ColumnIndex].Value != null)
                {
                    kode = selectedRow.Cells[0].Value.ToString();
                    InputNama.Text = selectedRow.Cells[1].Value.ToString();
                    InputUsername.Text = selectedRow.Cells[2].Value.ToString();
                    InputEmail.Text = selectedRow.Cells[3].Value.ToString();
